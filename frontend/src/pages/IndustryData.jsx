...
// ── Supply chain categories order and Chinese names ──
...
function SourceStatusBadge({ source, lastUpdated, status }) {
  const statusColors = {
    ...
    stale: "var(--warning, #eab308)",
    outdated: "var(--error, #ef4444)",
    never: "var(--text-secondary, #888)",
  };

  return (
    <span
      title={`${source}: ${status === "ok" ? "今日更新" : status === "stale" ? "3日内更新" : status === "outdated" ? "超3日未更新" : "从未采集"}`}
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 4,
...
function IndicatorCard({ indicator, onExpand, isExpanded }) {
  ...
  const changeDisplay = change_pct != null ? `${change_pct > 0 ? "+" : ""}${change_pct.toFixed(1)}%` : null;

  return (
    <div
      onClick={() => hasData && onExpand?.(indicator)}
      style={{
      ...
      onMouseEnter={(e) => { if (hasData) e.currentTarget.style.borderColor = "var(--accent, #3b82f6)"; }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4, fontSize: 12 }}>
            {changeDisplay && (
              <span style={{ color: change_pct >= 0 ? "var(--success, #22c55e)" : "var(--error, #ef4444)", fontWeight: 600 }}>
                {change_pct >= 0 ? "▲" : "▼"} {changeDisplay}
              </span>
            )}
            {latest_date && (
              <span style={{ color: "var(--text-secondary, #94a3b8)", fontSize: 11 }}>
                {latest_date.slice(0, 7)}
              </span>
            )}
            {data_quality === "estimated" && (
              <span style={{ color: "var(--warning, #eab308)", fontSize: 10 }}>估算</span>
            )}
          </div>
        </>
      ) : (
...
function IndustryData() {
  ...
  // Group indicators by category in supply chain order
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 12, color: "var(--text-secondary, #94a3b8)" }}>
            最后更新: {latestUpdate ? new Date(latestUpdate).toLocaleDateString("zh-CN") : "无数据"}
      ...
      <div style={{
        ...
        {dataSources.length === 0 && (
          <span style={{ fontSize: 12, color: "var(--text-secondary, #94a3b8)" }}>暂无数据源信息</span>
        )}
      </div>
